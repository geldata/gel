...
def fini_expression(
    ...
    server_conversions, server_conversion_params = (
        collect_param_conversions(ctx)
    )
    ...
    result = irast.Statement(
        ...
        server_conversions=server_conversions,
        server_conversion_params=server_conversion_params,
...
def collect_param_conversions(
    ...
    lparams = [
        (param_name, conversion_name, p)
        for param_name, conversions in ctx.env.server_param_conversions.items()
        for conversion_name, (_, p) in conversions.items()
    ...
    # Now flatten it out, including all sub_params, making sure subparams
    # appear in the right order.
    for param_name, conversion_name, p in lparams:
        conversions.append((param_name, conversion_name))
        params.append(p)
        if p.sub_params:
            params.extend(p.sub_params.params)
