...
def _infer_index_type(
    ...
    *, ctx: context.ContextLevel,
    ...
    str_t = env.schema.get_by_name(sn.QualName('std', 'str'))
    assert isinstance(str_t, s_scalars.ScalarType)
    bytes_t = env.schema.get_by_name(sn.QualName('std', 'bytes'))
    assert isinstance(bytes_t, s_scalars.ScalarType)
    int_t = env.schema.get_by_name(sn.QualName('std', 'int64'))
    assert isinstance(int_t, s_scalars.ScalarType)
    json_t = env.schema.get_by_name(sn.QualName('std', 'json'))
