...
def _get_ai_descs(extension_refs: Optional[Mapping[str, Any]]) -> tuple[
    ...
    if (
        ...
        and (reference_json := extension_refs.get('ai', None))
        and (
    ...
    *,
    ...
    if ':' in model_name:
        ...
        if provider_desc is None:
            raise errors.SchemaDefinitionError(
                f"Invalid model uri, unknown provider '{provider_label}'"
            )
    ...
    if (
        ...
        and model_provider != model_properties.model_provider
    ):
        # The URI will specify "openai" instead of "builtin::openai"
        # We want to show only "openai" in the case of an error here.
        model_provider_label = provider_label
        for provider_label, provider_desc in provider_descs.items():
            if provider_desc.name == model_provider:
                model_provider_label = provider_label
                break

        raise errors.SchemaDefinitionError(
            f"An embedding model with the name '{model_provider}' "
            f"exists but the provider specified by the index "
            f"('{model_name}') differs from the one "
