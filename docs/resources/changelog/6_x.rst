...
* Fix ``__default__`` when nested statements have defaults, and for link properties
  (:eql:gh:`#9006`)

* Fix casting to multi-ranges accidentally aggregating
  (:eql:gh:`#8995`)

* Fix losing overlays when iterating over DML with ``FOR``
  (:eql:gh:`#8994`)

* Fix some new simple_scoping + inline functions regressions
  (:eql:gh:`#8987`)

* Fix ``reindex(Obj.ptr)`` with ``future simple_scoping``
  (:eql:gh:`#8986`)
...
* Fix handling of SQL syntax error over gel protocol
  (:eql:gh:`#8961`)
...
* Fix auth extension email header and alternative content bug
  (:eql:gh:`#8919`)
