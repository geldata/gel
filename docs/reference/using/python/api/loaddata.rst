...
One of thefirst things that a project often needs is to load some data into the database. When you're just getting started, this data often comes in the form of JSON or CSV files.
...
The more likely scenario is that the JSON data exported from elsewhere potentially has duplicates (because some records are linked from multiple sources). So consider some data similar to the previous example, but with a few key differences. This time each user can be a memebr in more than one team. Also instead of ``"custom_settings"`` there are some themes that user can pick from:
...
If you're dealing with CSV files, you probably don't have any nested data there, instead records reference each other by prmary keys. Same processing would apply to JSON data linked by primary keys instead of nesting whole objects. So we'll use JSON in our example.
...
.. code-block:: python
  ...
  db = gel.create_client()
  # We're going to use dics to load the data
  teams = dict()
  users = dict()
  themes = dict()
